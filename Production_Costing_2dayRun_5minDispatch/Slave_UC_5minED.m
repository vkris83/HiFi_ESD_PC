...
fid=fopen('arcsinitial_5minEDUC.txt');
...
fid=fopen('C:\tomlab\problem_UC_5minED.mps', 'w');
...
for k1=1:length(arcstmp(:,1))  % Oct 12 2012 - Comp limit - disjoint
   if (strncmp(cell2mat(arcstmp{k1,2}),'ST',2)~=1)&&(strncmp(cell2mat(arcstmp{k1,3}),'ST',2)==1)
...
%         for mu_c=1:UT(count,1)-1
        for mu_c=1:str2double(cell2mat(arcstmp{k1,33}))-1
            if (loop_c-mu_c)>0
                fprintf(fid,' %16s',strcat('U',cell2mat(arcstmp{k1,1})));
...
%         for md_c=1:DT(count,1)-1
        for md_c=1:str2double(cell2mat(arcstmp{k1,34}))-1
            if (loop_c-md_c)>0
                fprintf(fid,' %16s',strcat('U',cell2mat(arcstmp{k1,1})));
...
%         fprintf(fid,' %25.4f',-min(UT(count,1),loop+1-loop_c));
        fprintf(fid,' %25.4f',-min(str2double(cell2mat(arcstmp{k1,33})),loop+1-loop_c));
        fprintf(fid,'\n'); 
...
for k1=1:length(arcstmp(:,1))  % This part define the startup and shutdown cost
    if (strncmp(cell2mat(arcstmp{k1,4}),'Gen',3)==1)&&(strncmp(cell2mat(arcstmp{k1,4}),'GenW',4)~=1)
        ...
        fprintf(fid,' %25.4f',-min(str2double(cell2mat(arcstmp{k1,34})),loop+1-loop_c));
        fprintf(fid,'\n'); 
...
% cd ('C:\tomlab')
% startup;
[F_UC,c_UC,A_UC,b_L_UC,b_U_UC,x_L_UC,x_U_UC,IntVars_UC] = cpx2mat('C:\tomlab\problem_UC_5minED.mps',0);
