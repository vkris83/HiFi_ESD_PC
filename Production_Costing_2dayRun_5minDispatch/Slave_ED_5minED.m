...
%**************************************************************************
% SCED at 5-min intervals (uses load5minly.mat, Reg_req5minED.mat)
...
dr_5=0;
lol=300;
num_x_5=0;  % dummy fuel resource nodes
num_gen_5=0;  % num. of generators
num_fuel_5=0; % num. of fuel transportation lines (excuding dummy lines)
num_dm_5=0;   % num. of demand nodes
num_nodes_5=0; % num. of all nodes except dummy nodes
num_arcs_5=0;% num. of all arcs
num_br_5=0; % number of existing electric branches
% num_fconst=0; % number of flow constraints
num_bus_5=0; % num. of all angles/buses
num_new_5=0; % num. of all potential lines
num_new_DC_5=0;
num_wind_5=0; % num. wind generators
num_wind_new_5=0; % num. wind generators
num_UC_5=0;

st_arcs_5=1;
st_x_5=0;          %num_arcs - num_dm - num_br - num_gen - num_x +1;
st_gen_5=0;        %st_x + num_x;
st_br_5=0;         %st_gen + num_gen;
st_dm_5=0;         %st_br + num_br;
st_new_5=0;
st_new_DC_5=0;
st_wind_5=0;
num_xcew_5=0;
...
obj_UC_5=0;         % The beginning of the UC variable
obj_UC_num_5=0;
obj_pointer_5=0;
obj_enebid_5=0;
obj_reserve1_5=0;   % The beginning of the first reserve1(spin) variable 
obj_reserve2_5=0;   % The beginning of the first reserve2(non-spin) variable 
...
expand_num_5=C{1,2}(1)*C{1,2}(2);

num_itg_5=0;      % number of integer variable
obj_row_ED={};
obj_variable_ED={};
obj_count_ED=0; 
row_count_ED=0;
...
fid=fopen('arcsinitial_5minED.txt');
arcsinitial=textscan(fid,'%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s' ,-1);  % 34 colmns
fclose('all');
...
fid=fopen('C:\tomlab\problem_ED_5minED.mps', 'w');
% fid=fopen('C:\Users\vkrish\Documents\MATLAB\tomlab\problem_ED.mps', 'w');

%   Write section ROWS
fprintf(fid,'NAME IES\nROWS\n N obj\n');
num_arcs_5=length(arcstmp(:,1)); % num. of all arcs
num_nodes_5=length(nodestmp(:,1)); % num. of all nodes
Name={};
fg01=0;
fg02=0;
fg03=0;
...
for k1=1:length(nodestmp(:,1))
    ...
    if strncmp(cell2mat(nodestmp{k1,1}),'EL',2)==1 % If it is elec node
        num_bus_5=num_bus_5+1;
        node_tran_5=node_tran_5+1;
        if fg01==0
            nst_tran_5=k1;
            fg01=1;
        end
...
for k1=1:length(arcstmp(:,1))
    if (strncmp(cell2mat(arcstmp{k1,2}),'EL',2)==1)&&(strncmp(cell2mat(arcstmp{k1,3}),'EL',2)==1)...
        ...
        num_br_5=num_br_5+1;
        Name{num_br_5,1}=strcat('BR',cell2mat(arcstmp{k1,1}));
        row_count_ED=row_count_ED+1;
    ...
    if (strncmp(cell2mat(arcstmp{k1,4}),'GenW',4)==1)&&(str2double(cell2mat(arcstmp{k1,10}))>10^-5)
        num_wind_new_5=num_wind_new_5+1;
        if flag09==0
        st_wind_new_5=k1;
        flag09=1;
        end
...
if st_wind_5==0
    st_wind_5=st_wind_new_5;
end
    
nst_wind_new_5=nst_wind_5+num_wind_5;
num_br_5=num_br_5-num_new_5;  % In this way, num_br is the num. of all existing lines
invcost=zeros(num_new_5,1); % the investment cost vector
line_num=zeros(num_new_5,1); % number of lines for each kind of potential lines
line_sus=zeros(num_new_5,1); % susceptance of each potential line
bound=zeros(num_new_5,1);   % bound of each potential line
for i_tmp=st_new_5:st_new_5+num_new_5-1
    j_tmp=i_tmp-st_new_5+1;
...
bb={};
for k4=1:a_5*b_5
    bb{k4,1}={[cell2mat(W(1)) num2str(fix((k4-1)/b_5)+1,'%02.0f')]};
end 
for k4=1:a_5
    for k5=1:b_5
        bb{(k4-1)*b_5+k5,1}={[cell2mat(bb{(k4-1)*b_5+k5,1}) cell2mat(W(2)) num2str(k5,'%02.0f')]};
    end
end
...
for k1=1:length(arcstmp(:,1))  
   if (strncmp(cell2mat(arcstmp{k1,2}),'ST',2)==1)&&(strncmp(cell2mat(arcstmp{k1,3}),'ST',2)==1)
       ...
       len_arc=length(cell2mat(arcstmp{k1,3}));
        ...
        if strncmp(arc_tmp(len_arc-5:len_arc),strcat('h',sprintf('%02.0f',a_5),'m',sprintf('%02.0f',b_5)),6)==1                    
...
% Oct 12 2012 - Comp limit - disjoint
...
%     % the sequence of row name appearing in "section ROWS" of mps file.
    ...
    if (strncmp(cell2mat(arcstmp{k1,2}),'EL',2)==1)&&(strncmp(cell2mat(arcstmp{k1,3}),'EL',2)==1)...
        ...
        Num_ang_5=Num_ang_5+1;
        Angle_tmp(Num_ang_5,1)={strcat('AG',cell2mat(arcstmp{k1,2}))};
        Angle_tmp(Num_ang_5,2)={strcat('BR',cell2mat(arcstmp{k1,1}))};
        Angle_tmp(Num_ang_5,3)={num2str(-bij)};
        Num_ang_5=Num_ang_5+1;
        Angle_tmp(Num_ang_5,1)={strcat('AG',cell2mat(arcstmp{k1,3}))};
        Angle_tmp(Num_ang_5,2)={strcat('BR',cell2mat(arcstmp{k1,1}))};
        Angle_tmp(Num_ang_5,3)={num2str(bij)};
...
for k1=1:length(arcstmp(:,1))  % This part adds two rows for each generator arc
    if (strncmp(cell2mat(arcstmp{k1,4}),'Gen',3)==1)&&(strncmp(cell2mat(arcstmp{k1,4}),'GenW',4)~=1)
        ...
        num_res1_5=num_res1_5+1;
        if flag_reserve1==0 
            st_spin_5=obj_count_ED;
            flag_reserve1=1;            
        end
        
        if strncmp(cell2mat(arcstmp{k1,4}),'GenS',4)==1
            num_res1_caes_5=num_res1_caes_5+1;
            if flag_reserve1_Caes==0 
                st_caes_spin_5=obj_count_ED;
...
%        fprintf(fid,'              obj %25.4f',30);
        ...
        obj_variable_ED{obj_count_ED,2}={num2str(obj_count_ED)};
        if flag_obj_RU ==0
            obj_RU_5 =obj_count_ED;
            flag_obj_RU =1;
        end
        num_reg_5 = num_reg_5+1;
        
        if strncmp(cell2mat(arcstmp{k1,4}),'GenS',4)==1
            if flag_reg_Caes==0 
                st_caes_reg_5=obj_count_ED;
                flag_reg_Caes=1;
            end
        end
...
for k1=1:length(arcstmp(:,1))
    if (strncmp(cell2mat(arcstmp{k1,2}),'ST',2)~=1)&&(strncmp(cell2mat(arcstmp{k1,3}),'ST',2)==1)
        ...
        obj_variable_ED{obj_count_ED,1}=strcat('A',cell2mat(arcstmp{k1,1}));
        obj_variable_ED{obj_count_ED,2}={num2str(obj_count_ED)};
        num_comA_5 = num_comA_5+1;
        if flag_obj_comA==0
            obj_comA_5=obj_count_ED;
            flag_obj_comA=1;
        end
...
%        fprintf(fid,'              obj %25.4f',30);
        ...
        obj_variable_ED{obj_count_ED,1}=strcat('RU',cell2mat(arcstmp{k1,1}));
        obj_variable_ED{obj_count_ED,2}={num2str(obj_count_ED)};
        num_comRU_5 = num_comRU_5+1;
        if flag_obj_comRU==0
            obj_comRU_5=obj_count_ED;
            flag_obj_comRU=1;
        end
...
%        fprintf(fid,'              obj %25.4f',30);
        ...
        obj_variable_ED{obj_count_ED,1}=strcat('RD',cell2mat(arcstmp{k1,1}));
        obj_variable_ED{obj_count_ED,2}={num2str(obj_count_ED)};
        num_comRD_5 = num_comRD_5+1;
        if flag_obj_comRD==0
            obj_comRD_5=obj_count_ED;
            flag_obj_comRD=1;
        end
...
load Reg_req5minED
% load Regh % Apr 10
%NLVhr = NLVhr(1:dpi_5,1);%% new
NLVhr = NLVar5(1:dpi_5,1);%% new
...
%         if strncmp(arc_tmp(len_arc-5:len_arc),strcat('h',sprintf('%02.0f',1),'m',sprintf('%02.0f',1)),6)==1
        if strncmp(arc_tmp(len_arc-2:len_arc),strcat('m',sprintf('%02.0f',1)),3)==1
            fprintf(fid,'\n rhs %15s%s',cell2mat(arcstmp{k1,1}),'R'); % Mar 30
            fprintf(fid,' %25f',str2double(cell2mat(arcstmp{k1,14})));           
            
            fprintf(fid,'\n rhs %15s%s',cell2mat(arcstmp{k1,1}),'D');
            fprintf(fid,' %25f',str2double(cell2mat(arcstmp{k1,15})));
        else
            fprintf(fid,'\n rhs %15s%s',cell2mat(arcstmp{k1,1}),'R'); % Mar 30
            fprintf(fid,' %25f',str2double(cell2mat(arcstmp{k1,14}))/12);
            
            fprintf(fid,'\n rhs %15s%s',cell2mat(arcstmp{k1,1}),'D');
            fprintf(fid,' %25f',str2double(cell2mat(arcstmp{k1,15}))/12);
        end
    end
end
...
% load loadhourly
% loadhr = loadhr(1:dpi_5,1);% new

load load5minly
loadhr = load_5min(1:dpi_5,1);% new
...
% cd ('C:\tomlab')
% startup;
[F_ED,c_ED,A_ED,b_L_ED,b_U_ED,x_L_ED,x_U_ED,IntVars_ED] = cpx2mat('C:\tomlab\problem_ED_5minED.mps',0);
% A=sparse(A);
F_ED=sparse(F_ED);
